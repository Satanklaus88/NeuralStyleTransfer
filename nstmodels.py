...
class StyleLoss(nn.Module):
    def __init__(self, target_feature):
        ...
        self.loss = F.mse_loss(self.target, self.target)# to initialize with someth
    def forward(self, input):
        G = gram_matrix(input)
...
def get_2style_model_and_losses(cnn, style_img, style_img2, content_img,
...
def run_2style_transfer(cnn, content_img, style_img, style_img2, input_img, 
                        content_layers_my,style_layers_my,num_steps=500,
                        style_weight=100000, style_weight2=100000, content_weight=1,verbose=True):
    """Run the style transfer."""
    print('Building the style transfer model..')
    model, style_losses, style_losses2, content_losses = get_2style_model_and_losses(cnn,
        style_img, style_img2, content_img,
        content_layers=content_layers_my,style_layers = style_layers_my)
...
def get_1style_model_and_losses(cnn, style_img, content_img,
                                content_layers=['conv_4'],
                                style_layers=['conv_1', 'conv_2', 'conv_3', 'conv_4']):
    cnn = copy.deepcopy(cnn)
    # normalization module
...
def run_1style_transfer(cnn, content_img, style_img, input_img,
                        content_layers_my,style_layers_my, num_steps=500,
                        style_weight=100000, content_weight=1,verbose=True):
    """Run the style transfer."""
    print('Building the style transfer model..')
    model, style_losses, content_losses = get_1style_model_and_losses(cnn,
        style_img, content_img,content_layers=content_layers_my,style_layers = style_layers_my)
    optimizer = get_input_optimizer(input_img)
    print('Optimizing..')
    run = [0]
    stopper = []
